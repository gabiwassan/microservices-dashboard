...
export const ServiceGroups: React.FC<ServiceGroupsProps> = ({
  ...
  const [processingGroups, setProcessingGroups] = useState<{ [key: string]: { isProcessing: boolean; action: 'start' | 'stop' | null } }>({});
  ...
  useEffect(() => {
    if (navigation.formData) {
      ...
      if (groupId && (actionType === "startGroup" || actionType === "stopGroup")) {
      ...
      const hasProcessingGroups = Object.values(processingGroups).some(g => g.isProcessing);
  ...
  }, [navigation.formData, revalidator]);
    ...
    <div className="w-[800px] bg-white dark:bg-gray-800 text-base">
      ...
      <div className="mb-6">
        <div className="flex gap-3">
          ...
          >
            {groups.map((group) => (
              <Select.Option key={group.id} value={group.id} className="text-base">
                {group.name}
              </Select.Option>
            ))}
          </Select>
          ...
          {selectedGroup && (
            ...
            >
              {services
                ...
                .map((service) => (
                  <Select.Option key={service.id} value={service.id} className="text-base">
                    {service.name}
                  </Select.Option>
                ))}
            </Select>
      ...
      <div className="space-y-6">
        {groups.map((group) => (
          <div key={group.id} className="p-6 border rounded-lg bg-white dark:bg-gray-800 dark:border-gray-700">
            <div className="flex items-center justify-between mb-4">
              {editingGroup === group.id ? (
                ...
                <div className="flex items-center gap-3">
                  <span className="text-lg font-medium text-gray-900 dark:text-white">{group.name}</span>
                  <Button
                    type="text"
                    size="large"
                    icon={<EditOutlined />}
                ...
                <button
                  className={`
                    ...
                    ${processingGroups[group.id]?.isProcessing 
                      ...
                      : group.services.some(sid => services.find(s => s.id === sid)?.status === 'running')
                ...
                >
                  {processingGroups[group.id]?.isProcessing ? (
                    <span className="flex items-center justify-center gap-1.5">
                      ...
                      <span>{processingGroups[group.id]?.action === 'start' ? 'Starting' : 'Stopping'}</span>
                    </span>
                  ) : (
                    <span className="flex items-center justify-center gap-1.5">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {group.services.length === 0 && (
                <span className="text-base text-gray-500 dark:text-gray-400">No services assigned</span>
              )}
            </div>
          </div>
        ))}
  ...
  return (
    <Popover
      content={content}
      title={<div className="text-lg font-medium text-gray-900 dark:text-white">Service Groups</div>}
      trigger="click"
      placement="bottomLeft"
      overlayClassName="dark:bg-gray-800 dark:border-gray-700"
      overlayInnerStyle={{ background: 'inherit' }}
    >
      <Button icon={<TeamOutlined />} size="large">Groups</Button>
    </Popover>
  );
}; 