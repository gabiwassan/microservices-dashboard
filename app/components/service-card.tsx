...
export default function ServiceCard({ service }: ServiceCardProps) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg shadow-gray-200/50 dark:shadow-gray-900/50 
      ...
      <div className="p-5">
        <div className="flex justify-between items-start gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Port: <span className="font-mono bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded">{service.port}</span>
            </p>
          </div>
          <Link
        ...
        <div className="mt-4 grid grid-cols-2 gap-4">
          <div className="space-y-1">
            <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Last Started</p>
            <p className="text-sm text-gray-900 dark:text-gray-100">
              {service.lastStarted ? new Date(service.lastStarted).toLocaleString() : "Never"}
            </p>
          </div>
          <div className="space-y-1">
            <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Last Stopped</p>
            <p className="text-sm text-gray-900 dark:text-gray-100">
              {service.lastStopped ? new Date(service.lastStopped).toLocaleString() : "Never"}
            </p>
          </div>
        </div>
      </div>
          ...
          >
            {isProcessing ? (
              <span className="flex items-center justify-center gap-1.5">
                ...
                <span>{processingAction === "start" ? "Starting" : "Stopping"}</span>
              </span>
            ) : (
              <span className="flex items-center justify-center gap-1.5">
                {isRunning ? (
